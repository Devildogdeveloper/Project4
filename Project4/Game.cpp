...
int Game::Score(vector<int>& dicePool, int die, array<int, 6>& dieCount, array<int, 6>& keptDice)
{
	...
	/*If ther are 2 dice of the selected value, set aside 2 of the dice

	if (dieCount[dieValue - 1] >= 2) {
		int i = 0;
		while (i < 2) {
			for (unsigned int a = 0; a < dicePool.size(); a++) {
				if (dicePool[a] == dieValue) {
					dicePool.erase(dicePool.begin() + a);
					keptDice[dieValue - 1]++;
					i++;
				}
			}
